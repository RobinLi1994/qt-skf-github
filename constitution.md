...
**核心：** 遵循C++语言的“少即是多”哲学。绝不进行不必要的抽象，绝不引入非必需的依赖。

- **1.1 (YAGNI):** 只实现`spec.md`中明确要求的功能。
-  **1.2 (标准库优先):** 必须优先使用 C++ 标准库，其次使用 Qt 提供的基础设施。
...
- **2.3 (谨慎使用 Mocks):** 默认使用真实依赖，仅在外部系统不可控时才允许 Mock。
...
- **3.1 (错误处理):** **不可协商**：所有错误都必须被显式处理。
  - 使用返回值或 `QResult / QVariant / enum` 表达失败状态
  - 禁止静默失败
  - 错误必须携带上下文信息（函数名 / 操作描述）
